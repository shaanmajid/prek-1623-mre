...
CSV_FILE="/results/${SAFE_PLATFORM}_${INSTALL_METHOD}_${SAFE_SOURCE}_${ITERATION}.csv"
...
if [ "$UV_SOURCE" = "auto" ]; then
  ...
  if timeout "$TIMEOUT_SECS" env PREK_UV_SOURCE="$UV_SOURCE" RUST_LOG="prek::languages::python::uv=trace" sh -lc "$PREK_CMD run" >/tmp/run.out 2>/tmp/run.err; then
    rc=0
  else
    rc=$?
  fi
...
selected_source="explicit:${UV_SOURCE}"
if [ "$UV_SOURCE" = "auto" ]; then
...
printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
  "$PLATFORM" "$INSTALL_METHOD" "$UV_SOURCE" "$ITERATION" "$selected_source" "$rc" "$uv_exec" "$link_type" "$uv_version" "$error_sig" \
  >> "$CSV_FILE"
